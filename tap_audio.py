...
async def main():
    ...
    try:
        ...
        await frame.upload_stdlua_libs(lib_names=['data', 'code', 'audio', 'tap'])
        ...
        while True:
            ...
            if not recording:
                ...
                try:
                    # Wait for audio data with a longer timeout since we need to collect all the data
                    print("Collecting audio data...")
                    audio_samples = await asyncio.wait_for(audio_queue.get(), timeout=5.0)
